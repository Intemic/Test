package example; //: containers/ListPerformance.java
    ...
    static {
        tests.add(new Test<List<String>>("add") {
            public int test(List<String> list, TestParam tp) {
                ...
                Generator<String> gn = new CountingGenerator.String();
        ...
        tests.add(new Test<List<String>>("set") {
            public int test(List<String> list, TestParam tp) {
                ...
                int listSize = list.size();
                for (int i = 0; i < loops; i++)
                    list.set(rand.nextInt(listSize), new CountingGenerator.String().next()); //47);
                return loops;
            }
        });
        tests.add(new Test<List<String>>("iteradd") {
            public int test(List<String> list, TestParam tp) {
                ...
                for (int i = 0; i < LOOPS; i++)
                    it.add(new CountingGenerator.String().next()); //(47);
                return LOOPS;
            }
        });
        tests.add(new Test<List<String>>("insert") {
            public int test(List<String> list, TestParam tp) {
                int loops = tp.loops;
                for (int i = 0; i < loops; i++)
                    list.add(5, new CountingGenerator.String().next()); //47); // Minimize random-access cost
                return loops;
            }
        });
